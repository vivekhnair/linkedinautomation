...
def api_calendar():
    return jsonify(generate_content_calendar(analyze_performance()))

@app.route("/api/leads")
def api_leads():
    return jsonify(get_lead_recommendations(analyze_performance()))
...
def api_generate_report_stream():
    ...
    if not api_key:
        return Response(stream_with_context(iter(["ERROR: OpenAI API key not set.\n\nPaste your key at the top and click Save Key."])), mimetype="text/plain")
...
{'Focus topic: ' + topic if topic else 'Auto-select the highest-potential topic for today.'}
...
SECTION 4: CONTENT OPPORTUNITIES (5 ideas with buyer, format, why now)
SECTION 5: 7-DAY CONTENT CALENDAR (Mon-Fri, IST times, specific topics)
...
SECTION 11: VIDEO CONCEPT (30-60 sec script)
SECTION 12: EMPLOYEE ADVOCACY (CEO, CTO, VP Marketing, Sales Leader versions)
SECTION 13: PUBLISHING RECOMMENDATION (exact time IST, format, audience)
...
SECTION 17: ACTIONS BEFORE NEXT POST (checklist)"""
...
def api_generate_comment_reply():
    ...
    d = request.json
    try:
        return jsonify({"content": generate_comment_response(d.get("post",""), d["comment"], d.get("title",""))})
    except Exception as e:
        return jsonify({"error": str(e)})
